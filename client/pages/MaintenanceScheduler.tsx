...
import { useMaintenanceSchedulerFeatures } from '@/hooks/useFeatureMatrix';
...
// Sample data for Free and Basic plans
const sampleTasks: MaintenanceTask[] = [
  {
    id: 'TASK-SAMPLE-001',
    title: 'Sample Pothole Repair - Demo Street',
    description: 'Demo task showing pothole repair workflow',
    assetId: 'ROAD-SAMPLE-001',
    assetName: 'Demo Street (Sample)',
    contractor: 'Sample City Crew',
    priority: 'high',
    status: 'scheduled',
    ...
    assignedCrew: 'Sample Maintenance Team',
    pciScore: 45,
    isSampleData: true
  },
  {
    id: 'TASK-SAMPLE-002',
    title: 'Sample Crack Sealing - Demo Avenue',
    description: 'Demo preventive maintenance task',
    assetId: 'ROAD-SAMPLE-002',
    assetName: 'Demo Avenue (Sample)',
    contractor: 'Sample ABC Paving',
    priority: 'medium',
    status: 'in-progress',
    ...
    weatherSensitive: true,
    assignedCrew: 'Sample Crew 2',
    pciScore: 72,
    isSampleData: true
  }
];

// Real data for Pro+ plans
...
export default function MaintenanceScheduler() {
  const maintenanceSchedulerFeatures = useMaintenanceSchedulerFeatures();
  const { userPlan } = useDataVisibility();
  
  // Determine data source and features based on plan
  const tasks = maintenanceSchedulerFeatures.functionality.useRealData ? realTasks : sampleTasks;
  const { isPreviewMode, requiredPlan } = usePreviewMode(maintenanceSchedulerFeatures.functionality.state, userPlan);
  ...
  const addNewTask = async () => {
    if (!maintenanceSchedulerFeatures.functionality.useRealData) {
      alert('Creating new maintenance tasks requires Pro plan or higher with real data access.');
      return;
    }
    // In real app, this would make API call to create task
    alert('New maintenance task would be created via API call to /api/maintenance');
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 mt-1">
              {maintenanceSchedulerFeatures.functionality.useRealData
                ? "Schedule, track, and assign maintenance tasks with weather-aware planning"
                : "Sample maintenance scheduling data - upgrade to Pro for full functionality"
              }
            </p>
      ...
      {/* Unlock Advanced Scheduling Features Card for Free and Basic Plans */}
      {maintenanceSchedulerFeatures.unlockAdvancedSchedulingCard.show && (
          ...
          <CardContent className="p-8">
            <div className="text-center">
              ...
              <p className="text-slate-600 dark:text-slate-300 mb-6 max-w-2xl mx-auto">
                {userPlan === 'free' 
                  ? "Upgrade to Pro plan to unlock advanced maintenance scheduling with real data integration, weather-aware planning, and unlimited contractor management."
                  : "Upgrade to Pro plan to unlock full maintenance scheduling functionality with advanced features and real-time data integration."
                }
              </p>
      ...
      {/* Main Scheduling Content */}
      <PreviewModeWrapper
        isPreviewMode={isPreviewMode}
        requiredPlan={requiredPlan}
        featureName="Maintenance Scheduling"
      >
        {/* Calendar View */}
        {viewMode === 'calendar' && (
          <Card className="glass-card border-white/20">
            <CardHeader>
              <CardTitle className="flex items-center text-slate-800 dark:text-white">
                ...
                Weekly Schedule
                {!maintenanceSchedulerFeatures.functionality.useRealData && (
                  <Badge className="ml-2 bg-blue-100 text-blue-700">Sample Data</Badge>
                )}
              </CardTitle>
              <CardDescription>
                {maintenanceSchedulerFeatures.functionality.useRealData
                  ? "Drag and drop tasks to reschedule. Weather icons indicate suitability for outdoor work."
                  : "Sample maintenance schedule data. Upgrade to Pro for full scheduling capabilities."
                }
              </CardDescription>
            </CardHeader>
            <CardContent>
